...
export function keyToLowercase(obj: Style) {
	...
	for (const prop in obj) {
		const key = stringToLowercase(prop as keyof Style) as keyof Style;
		//@ts-ignore
		objLc[key] = obj[prop as keyof Style];
	}
	return objLc;
}
...
export function stringToLowercase(str: string) {
...
export function diff<T>(prec: T, next: T) {
