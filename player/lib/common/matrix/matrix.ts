...
const skew = (α, β = α) => {
	α = toRad(α);
	β = toRad(β);
	return [1, tan(α), 0, tan(β), 1, 0, 0, 0, 1];
};

const skewX = (α) => {
	α = toRad(α);
	return [1, tan(α), 0, 0, 1, 0, 0, 0, 1];
};

const skewY = (β) => {
	β = toRad(β);
	return [1, 0, 0, tan(β), 1, 0, 0, 0, 1];
};
...
const scaleX = (scalex) => {
	return [scalex, 0, 0, 0, 1, 0, 0, 0, 1];
};

const scaleY = (scaley) => {
	return [1, 0, 0, 0, scaley, 0, 0, 0, 1];
};
...
const rotate = (deg, cx = 0, cy = cx) => {
	if (deg !== 0) {
		...
		const m2 = [cos(α), -sin(α), 0, sin(α), cos(α), 0, 0, 0, 1];
