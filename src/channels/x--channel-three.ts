...
export class ThreeChannel extends Channel {
	...
	callback: Array<(event: RunEvent) => void> = [];
	onTick(status) {
		animate(status);
	}
	run(event: RunEvent): void {
		this.callback.forEach((fn) => fn(event));

		const { name, time, status, data } = event;
...
//  a placer dans le store du channel
